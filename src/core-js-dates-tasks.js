...
function getNextFriday(date) {
  const MS_IN_DAY = 86400e3;
  const FRIDAY_INDEX = 6;
  const dateIndex = (date.getDay() + 1) % 7;
  const deltaDays = FRIDAY_INDEX - dateIndex;
  return new Date(date.getTime() + (deltaDays || 7) * MS_IN_DAY);
}
...
function getCountDaysOnPeriod(dateStart, dateEnd) {
  const MS_IN_DAY = 86400e3;
  return (
    Math.round((Date.parse(dateEnd) - Date.parse(dateStart)) / MS_IN_DAY) + 1
  );
}
...
function getWeekNumberByDate(date) {
  ...
  const daysUntilNextMonday = 7 - firstDayIndex;

  let firstWeekStart;
  let fullWeeksStart;

  if (firstDayIndex > THURSDAY_INDEX) {
    firstWeekStart = new Date(firstDay);
    firstWeekStart.setDate(firstDay.getDate() + daysUntilNextMonday);

    fullWeeksStart = new Date(firstWeekStart);
    fullWeeksStart.setDate(firstWeekStart.getDate() + 7);
  } else {
    firstWeekStart = firstDay;
    fullWeeksStart = new Date(firstWeekStart);
    fullWeeksStart.setDate(firstWeekStart.getDate() + daysUntilNextMonday);
  }

  if (date >= firstWeekStart && date < fullWeeksStart) return 1;

  return Math.floor((date - fullWeeksStart) / (MS_IN_DAY * 7)) + 2;
}
...
function getNextFridayThe13th(date) {
  ...
  if (date.getDate() > 13) {
    searchDate.setMonth(date.getMonth() + 1);
  }

  while (searchDate.getDay() !== FRIDAY_INDEX) {
...
function getWorkSchedule(period, countWorkDays, countOffDays) {
  const parseDate = function parse(dateStr) {
    const [day, month, year] = dateStr.split('-');
    return new Date(year, month - 1, day);
  };

  const formatDateObj = function format(dateObj) {
  ...
  while (searchDate <= endDate) {
    for (let i = 0; i < countWorkDays; i += 1) {
      if (searchDate <= endDate) {
        workSchedule.push(formatDateObj(searchDate));
      }
      searchDate.setDate(searchDate.getDate() + 1);
    }
    searchDate.setDate(searchDate.getDate() + countOffDays);
  }
